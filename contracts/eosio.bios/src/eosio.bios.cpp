...
void bios::syscurrency() {
    ...
    name resource_token_contract = eosio::name("oncoin.tmy");
}
...
void bios::buyram(eosio::name dao_owner, eosio::name app, eosio::asset quant) {
   ...
   eosio::check(quant.amount > 0, "Amount must be positive");
    ...
    // Get the RAM price
   eosiobios::bios::resource_config_table resource_table(get_self(), get_self().value);
   auto resource_itr = resource_table.find(get_self().value);
    ...
    resource_table.modify(resource_itr, get_self(), [&](auto& row) {
